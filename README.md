...
- A draggable two-state Lumiverse float player: a square image orb and a full **Quick create** panel. Its visible surface mounts above Lumiverse's zero-height mobile body to avoid physical-device clipping; it survives closing Studio, generates from editable lightweight prompts, follows live previews and step progress, can append the latest output to chat, and turns its stable Generate action into Stop while rendering
