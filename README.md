...
The **Prompt family** switch beside the protocol editor selects the final subject serializer. **Anima** uses hybrid natural-language spatial clauses plus useful tags and richer per-subject appearance. **Illustrious** keeps a tighter tag-oriented form with exact count early and each subject's distinctive tokens adjacent. Neither mode relies on `BREAK` or claims to regionally isolate globally loaded LoRAs.
