...
class StudioController {
    ...
    renderLoras() {
        ...
        if (!items.length) {
            const text = this.state.loras.length ? "No LoRAs match this search." : "No LoRA metadata was returned. You can still add a model by filename.";
    ...
    generate() {
        ...
        this.send("generate", {
            input: {
                prompt,
                negativePrompt: this.get('[data-role="negative"]').value.trim() || undefined,
                connection_id: this.state.connection.id,
                model: this.get('[data-role="model"]').value || this.state.connection.model,
                clientJobId,
                parameters
            }
        });
    ...
    showLivePreview(src, step, totalSteps) {
        ...
        this.get('[data-role="preview-loading"]').dataset.visible = "false";
        this.get('[data-role="output-label"]').textContent = totalSteps > 0 ? `Live SwarmUI preview · ${step} / ${totalSteps}` : "Live SwarmUI preview";
