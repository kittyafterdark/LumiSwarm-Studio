...
export function matchesKeywordQuery(query, values) {
...
export function quickGenerationParameters(defaults = {}) {
...
export function dimensionsForAspect(aspect, scale) {
...
export function applyPresetPrompt(base, update) {
...
export function lorasFromSwarmPreset(paramMap) {
...
are removed when CSS is applied.</pre>
    ...
    openInspector() {
        ...
        this.get('[data-role="inspector-positive"]').textContent = details?.prompt || details?.resolvedPrompt || "Prompt metadata is unavailable for this older output.";
    ...
    renderOutputLibrary() {
        ...
        this.get('[data-role="output-library"]').dataset.selectionMode = String(this.librarySelectionMode);
        const filtered = this.filteredLibraryOutputs();
        const pages = Math.max(1, Math.ceil(filtered.length / this.currentLibraryPageSize()));
