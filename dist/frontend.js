...
function visualRecord(value) {
    return value && typeof value === "object" && !Array.isArray(value) ? value : {};
}
function visualStrings(value, limit = 100) {
    if (!Array.isArray(value)) return [];
    return value.map((item)=>String(item || "").trim()).filter(Boolean).slice(0, limit);
}
function cleanVisualLoras(value) {
    if (!Array.isArray(value)) return [];
    const seen = new Set();
    const output = [];
...
function cleanVisualProfiles(value) {
    if (!Array.isArray(value)) return [];
...
export function cleanLoomVisualRecipe(value) {
    const raw = visualRecord(value);
...
function parseVisualLoraLines(value) {
    ...
    if (typeof ctx.ui?.registerCharacterEditorTab === "function" && ctx.ui.characterEditor) {
        try {
            ...
            const render = (force = false)=>{
                ...
                copy.append(element("h2", "", "Character Visual Bible"), element("p", "", "A durable appearance recipe stored on this character card. Studio layers it over the current Loom visual style whenever this character is active."));
    ...
    if (typeof ctx.ui?.registerPresetEditorTab === "function" && ctx.ui.presetEditor?.extension) {
        try {
            ...
            const render = (force = false)=>{
                ...
                const header = element("header", "ss-visual-editor-head");
                const copy = element("div");
                copy.append(element("h2", "", "Loom Visual Style"), element("p", "", "Pair this text preset with a Swarm checkpoint, preset stack, LoRAs, and visual prompt additions. The recipe survives native duplicate and Loom export/import."));
...
class StudioController {
    ...
    constructor(ctx, modal, onThemeChange, onAppearanceChange, behavior, onBehaviorChange, activity = null, loomVisualContext = {
        id: "",
        name: "",
        recipe: null
    }){
...
are removed when CSS is applied.</pre>
    ...
    async setCurrentAsCharacterAvatar() {
        const image = this.state.currentImage;
    ...
    applyCurrentOutputPalette() {
        const image = this.state.currentImage;
        if (!image?.id) return;
        this.setRunStatus("Extracting a Lumiverse palette from the selected output…");
        this.send("apply_output_palette", {
            imageId: image.id
        });
    }
    setCurrentAsChatWallpaper() {
        const image = this.state.currentImage;
        ...
        this.send("set_output_as_chat_wallpaper", {
            imageId: image.id
        });
    }
    ...
    updateLumiverseOutputControls() {
        ...
        const avatar = this.root.querySelector('[data-action="set-character-avatar"]');
        if (avatar) {
            avatar.disabled = !(saved && hasCharacter && this.state.permissions.characters);
            avatar.title = avatar.disabled ? "Requires a saved output, an active character chat, and Characters permission" : "Replace the active character card avatar with this output";
        }
        const palette = this.root.querySelector('[data-action="apply-output-palette"]');
        if (palette) {
            palette.disabled = !(saved && this.state.permissions.theme && this.state.permissions.images);
            palette.title = palette.disabled ? "Requires a saved output plus Images and App Manipulation permissions" : "Apply an extension-scoped Lumiverse palette derived from this output";
        }
        const wallpaper = this.root.querySelector('[data-action="set-chat-wallpaper"]');
        if (wallpaper) {
            wallpaper.disabled = !(saved && hasChat && this.state.permissions.wallpaper);
            wallpaper.title = this.state.permissions.wallpaper ? "Set this output as the current chat wallpaper" : "Waiting for Lumiverse’s public chat wallpaper helper";
        }
        const note = this.root.querySelector('[data-role="lumiverse-action-note"]');
        if (note) {
            note.textContent = this.state.permissions.wallpaper ? "Lumiverse actions preserve the saved image and its Swarm Studio generation metadata." : "Avatar, chat, and palette actions use public Spindle APIs. Wallpaper stays disabled until Lumiverse exposes its public chat-wallpaper helper.";
        }
    }
    downloadCurrent() {
...
export function setup(ctx) {
    ...
    const openStudio = (initialView = "studio")=>{
        ...
        activeModal = modal;
        activeStudio = new StudioController(ctx, modal, selectTheme, updateAppearance, behavior, updateBehavior, miniplayer, visualEditors?.getPresetContext() || {
            id: "",
            name: "",
            recipe: null
        });
