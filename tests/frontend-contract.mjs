...
assert.match(source, /move\.dataset\.action = "move-library-output"/)
...
assert.match(source, /this\.isMobileViewport\(\)[\s\S]*?\? !this\.behavior\.mobileQuickCreate[\s\S]*?: stored\.collapsed === true/)
assert.match(source, /enabledMobileQuickCreate[\s\S]*?this\.setCollapsed\(false\)/)
assert.match(source, /this\.sizeTarget\.style\.setProperty\("width"/)
...
assert.match(source, /data-role="move-folder-modal"\]\s*\{\s*z-index:\s*2147483200/)
