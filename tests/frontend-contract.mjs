...
assert.match(source, /\.ss-miniplayer-app-surface\s*\{[\s\S]*?z-index:\s*2147483000/)
...
assert.match(source, /document\.documentElement\.appendChild\(surface\)/)
assert.match(source, /surface\.remove\(\)[\s\S]*?mount\.destroy\(\)/)
...
assert.match(source, /typeof this\.widget\.setVisible === "function"[\s\S]*?this\.root\.hidden = !this\.behavior\.widgetEnabled/)
