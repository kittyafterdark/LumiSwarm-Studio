...
const EXTENSION_NAMESPACE = "swarm_studio"
...
interface CharacterVisualBible {
  ...
  mode: "preset" | "custom"
  connectionId: string
  presetStack: SelectedPreset[]
  canonicalPrompt: string
  canonicalNegativePrompt: string
  preferredCheckpoint: string
  baseLoraStackId: string
  baseLoraStackName: string
  baseLoraStack: VisualLoraItem[]
...
interface LoomVisualRecipe {
  version: 1
  preferredCheckpoint: string
  swarmPresets: string[]
  loraStack: VisualLoraItem[]
  positiveAddition: string
  negativeAddition: string
  width: number | null
  height: number | null
  sceneExtractionStyle: string
}
...
interface StudioState {
  ...
  characterVisual: CharacterVisualBible | null
...
const STUDIO_V3_STYLES = `
  ...
  .ss-visual-editor {
    ...
    color: var(--lumiverse-text);
    background:
      radial-gradient(circle at 88% 4%, color-mix(in srgb, var(--lumiverse-primary) 11%, transparent), transparent 28%),
      var(--lumiverse-bg);
  }
  .ss-visual-editor-head { display: flex; align-items: flex-start; justify-content: space-between; gap: 14px; }
  .ss-visual-editor-head h2 { margin: 0; font: 600 20px/1.15 Georgia, serif; letter-spacing: .015em; }
  .ss-visual-editor-head p { max-width: 680px; margin: 5px 0 0; color: var(--lumiverse-text-muted); font-size: 11px; line-height: 1.5; }
  .ss-visual-save-state { color: var(--lumiverse-text-dim); font-size: 10px; white-space: nowrap; }
  .ss-visual-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 11px; }
  .ss-visual-card {
    min-width: 0;
    display: grid;
    gap: 9px;
  ...
  .ss-visual-textarea { min-height: 86px; resize: vertical; line-height: 1.45; }
  .ss-visual-input:focus,
  .ss-visual-select:focus,
  .ss-visual-textarea:focus { border-color: var(--lumiverse-primary); }
  ...
  .ss-use-lumiverse h4 { margin-top: 0; }
  .ss-use-lumiverse-actions { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; }
  .ss-use-lumiverse-note { margin: 7px 0 0; color: var(--lumiverse-text-dim); font-size: 9px; line-height: 1.4; }
...
function visualRecord(value: unknown): Record<string, any> {
  return value && typeof value === "object" && !Array.isArray(value)
    ? value as Record<string, any>
    : {}
}

function visualStrings(value: unknown, limit = 100): string[] {
...
function cleanVisualLoras(value: unknown): VisualLoraItem[] {
  if (!Array.isArray(value)) return []
  const seen = new Set<string>()
  const output: VisualLoraItem[] = []
  for (const entry of value.slice(0, 64)) {
    ...
    output.push({
      name,
      title: String(item.title || "").trim().slice(0, 300) || undefined,
      weight: Number.isFinite(weight) ? clamp(weight, -10, 10) : 1,
      enabled: item.enabled !== false,
      useTrigger: item.useTrigger === true,
      sourceUrl: String(item.sourceUrl || "").trim().slice(0, 2000) || undefined,
    })
  }
  return output
}

function cleanVisualProfiles(value: unknown): VisualProfile[] {
  if (!Array.isArray(value)) return []
...
function cleanVisualPresetStack(value: unknown): SelectedPreset[] {
  if (!Array.isArray(value)) return []
  const seen = new Set<string>()
  const output: SelectedPreset[] = []
  for (const entry of value.slice(0, 30)) {
    const item = visualRecord(entry)
    const title = (typeof entry === "string" ? entry : item.title)
      ? String(typeof entry === "string" ? entry : item.title).trim().slice(0, 300)
      : ""
    const key = title.toLowerCase()
    if (!title || seen.has(key)) continue
    seen.add(key)
    output.push({ title, enabled: typeof entry === "string" || item.enabled !== false })
  }
  return output
}

export function cleanCharacterVisualBible(value: unknown): CharacterVisualBible {
...
export function cleanLoomVisualRecipe(value: unknown): LoomVisualRecipe {
  const raw = visualRecord(value)
  const dimension = (input: unknown): number | null => {
    const value = Number(input)
    return Number.isFinite(value) && value >= 64 ? clamp(Math.round(value), 64, 4096) : null
  }
  return {
    version: 1,
    preferredCheckpoint: String(raw.preferredCheckpoint || "").trim().slice(0, 500),
    swarmPresets: visualStrings(raw.swarmPresets, 20).map((item) => item.slice(0, 300)),
    loraStack: cleanVisualLoras(raw.loraStack),
    positiveAddition: String(raw.positiveAddition || "").slice(0, 12000),
    negativeAddition: String(raw.negativeAddition || "").slice(0, 12000),
    width: dimension(raw.width),
    height: dimension(raw.height),
    sceneExtractionStyle: String(raw.sceneExtractionStyle || "").slice(0, 12000),
  }
}

function parseVisualLoraLines(value: string): VisualLoraItem[] {
  return cleanVisualLoras(value.split(/\r?\n/).map((line) => {
    const [name = "", weight = "1", trigger = ""] = line.split("|").map((part) => part.trim())
    return {
      name,
      weight: Number(weight),
      enabled: true,
      useTrigger: /^(1|true|yes|trigger)$/i.test(trigger),
    }
  }))
}

function formatVisualLoraLines(items: VisualLoraItem[]): string {
  return items.map((item) => `${item.name} | ${item.weight} | ${item.useTrigger ? "trigger" : ""}`.trimEnd()).join("\n")
}

function splitVisualLines(value: string): string[] {
  return value.split(/[\r\n,]+/).map((item) => item.trim()).filter(Boolean)
}

function visualField(label: string, control: HTMLElement): HTMLLabelElement {
  const field = element("label", "ss-visual-field")
  field.append(element("span", "", label), control)
  return field
}

function visualInput(value = "", placeholder = ""): HTMLInputElement {
  const input = element("input", "ss-visual-input")
  input.value = value
  input.placeholder = placeholder
  return input
}

function visualTextarea(value = "", placeholder = ""): HTMLTextAreaElement {
  const input = element("textarea", "ss-visual-textarea")
  input.value = value
  input.placeholder = placeholder
  return input
}

function visualCard(title: string, hint = "", wide = false): HTMLElement {
  const card = element("section", "ss-visual-card")
  card.dataset.wide = String(wide)
  const head = element("div", "ss-visual-card-head")
  head.append(element("strong", "", title), element("span", "", hint))
  card.appendChild(head)
  return card
}

interface VisualEditorOptions {
  connection: { id: string; name: string; model?: string; isDefault?: boolean } | null
  checkpoints: CheckpointMetadata[]
  swarmPresets: SwarmPreset[]
  stackPresets: StackPreset[]
...
interface VisualEditorActions {
  openStudio: (context: VisualLaunchContext) => void
  openLibrary: (folderId: string) => void
}

interface VisualLaunchContext {
  characterId: string
  characterName: string
  visualBible: CharacterVisualBible
}

function registerVisualRecipeEditors(ctx: FrontendContext, actions: VisualEditorActions): {
  getPresetContext: () => { id: string; name: string; recipe: LoomVisualRecipe | null }
  onBackendMessage: (payload: any) => void
  refresh: () => void
  destroy: () => void
} {
  let currentPresetId = ""
  let currentPresetName = ""
  let currentPresetRecipe: LoomVisualRecipe | null = null
  let currentCharacterId = ""
  let galleryCharacterId = ""
  let galleryFolderId = ""
  let galleryOutputs: any[] = []
  let galleryRoot: HTMLElement | null = null
  let rerenderCharacterEditor: (() => void) | null = null
  let optionsRequestId = ""
  let editorOptions: VisualEditorOptions = {
    connection: null,
    checkpoints: [],
    swarmPresets: [],
    stackPresets: [],
  ...
  const requestEditorOptions = () => {
    ...
    ctx.sendToBackend({ type: "visual_editor_options", requestId: optionsRequestId })
  }

  const requestCharacterGallery = (characterId: string) => {
    if (!characterId) return
    ctx.sendToBackend({
      type: "list_character_gallery",
  ...
  const renderGallery = () => {
    ...
    if (!galleryOutputs.length) {
      galleryRoot.appendChild(element("div", "ss-visual-gallery-empty", "Images generated for this character will collect here automatically."))
      return
    }
  ...
  if (typeof ctx.ui?.registerCharacterEditorTab === "function" && ctx.ui.characterEditor) {
    try {
    const tab = ctx.ui.registerCharacterEditorTab({ id: "visuals", title: "Visuals" })
    handles.push(tab)
    let renderedCharacterId = ""
    const render = (force = false) => {
      ...
      currentCharacterId = String(state.characterId)
      if (!force && renderedCharacterId === currentCharacterId && tab.root.childElementCount) return
      renderedCharacterId = currentCharacterId
      ...
      const shell = element("div", "ss-visual-editor")
      const header = element("header", "ss-visual-editor-head")
      const copy = element("div")
      copy.append(
        element("h2", "", "Character Visual Bible"),
        element("p", "", "A durable appearance recipe stored on this character card. Studio layers it over the current Loom visual style whenever this character is active."),
      )
      ...
      for (const [value, label, hint] of [
        ["preset", "Swarm presets", "Use an ordered stack of reusable SwarmUI presets."],
        ["custom", "Custom bible", "Keep this character's prompts, model, LoRAs, and aspect ratio here."],
      ] as const) {
      ...
      for (const [value, label] of [["", "Connection default"], ["1:1", "Square · 1:1"], ["2:3", "Portrait · 2:3"], ["3:2", "Landscape · 3:2"], ["9:16", "Tall · 9:16"], ["16:9", "Wide · 16:9"]]) {
      ...
      const buildProfiles = (items: VisualProfile[]) => {
        ...
        for (const [index, profile] of items.entries()) {
          ...
          for (const [field, value, placeholder] of [
            ...
            ["prompt", profile.prompt, "Positive additions"],
            ["negativePrompt", profile.negativePrompt, "Negative additions"],
          ] as const) {
      ...
      const write = (rerender = false) => {
        ctx.ui.characterEditor.updateExtensions((extensions: Record<string, any>) => ({
          ...extensions,
          [EXTENSION_NAMESPACE]: {
      ...
      shell.addEventListener("input", (event) => {
        ...
        if (field === "canonicalPrompt") recipe.canonicalPrompt = target.value
        if (field === "canonicalNegativePrompt") recipe.canonicalNegativePrompt = target.value
        if (field === "preferredCheckpoint") recipe.preferredCheckpoint = target.value
        ...
        const profileField = target.dataset.visualProfileField as keyof VisualProfile | undefined
        if (Number.isInteger(index) && profileField && recipe.profiles[index]) {
          ;(recipe.profiles[index] as any)[profileField] = target.value
        }
        write()
      })
      shell.addEventListener("change", (event) => {
        ...
        if (target.dataset.visualAddPreset) {
          ...
          if (title && !recipe.presetStack.some((item) => item.title.toLowerCase() === title.toLowerCase())) {
            recipe.presetStack.push({ title, enabled: true })
            write(true)
          }
          return
        }
      ...
      shell.addEventListener("click", (event) => {
        ...
        if (button.dataset.visualOpenStudio) {
          actions.openStudio({
            ...
            characterName: String((ctx.ui.characterEditor.getState() as any)?.character?.name || "Character"),
            visualBible: cleanCharacterVisualBible(recipe),
          })
          return
        }
        ...
        if (button.dataset.visualAddProfile) {
          recipe.profiles.push({ id: crypto.randomUUID(), name: "New profile", prompt: "", negativePrompt: "", enabled: true })
          write(true)
          return
        }
      ...
      galleryCharacterId = currentCharacterId
      galleryOutputs = []
      renderGallery()
    ...
    render()
      ...
      // A denied Characters placement must not prevent the independent Loom
      // Visual Style tab (or the rest of Studio) from registering.
    }
  }

  if (typeof ctx.ui?.registerPresetEditorTab === "function" && ctx.ui.presetEditor?.extension) {
    try {
    const tab = ctx.ui.registerPresetEditorTab({ id: "visual-style", title: "Visual Style" })
    handles.push(tab)
    let renderedPresetId = ""
    const render = (force = false) => {
      const editor = ctx.ui.presetEditor.extension
      const state = editor.getState()
      if (!state.open || !state.presetId) {
        renderedPresetId = ""
        tab.root.replaceChildren()
        return
      }
      currentPresetId = String(state.presetId)
      currentPresetName = String((ctx.ui.presetEditor.getState().preset?.name) || "Loom preset")
      const metadata = visualRecord(state.metadata)
      const recipe = cleanLoomVisualRecipe(metadata.visualRecipe)
      currentPresetRecipe = recipe
      if (!force && renderedPresetId === currentPresetId && tab.root.childElementCount) return
      renderedPresetId = currentPresetId
      const shell = element("div", "ss-visual-editor")
      const header = element("header", "ss-visual-editor-head")
      const copy = element("div")
      copy.append(
        element("h2", "", "Loom Visual Style"),
        element("p", "", "Pair this text preset with a Swarm checkpoint, preset stack, LoRAs, and visual prompt additions. The recipe survives native duplicate and Loom export/import."),
      )
      header.append(copy, element("span", "ss-visual-save-state", "Namespaced preset metadata"))
      shell.appendChild(header)
      const grid = element("div", "ss-visual-grid")
      const model = visualCard("Model and dimensions", "Visual defaults")
      const checkpoint = visualInput(recipe.preferredCheckpoint, "Checkpoint filename or title")
      checkpoint.dataset.recipeField = "preferredCheckpoint"
      const width = visualInput(recipe.width ? String(recipe.width) : "", "Width")
      width.type = "number"
      width.min = "64"
      width.max = "4096"
      width.dataset.recipeField = "width"
      const height = visualInput(recipe.height ? String(recipe.height) : "", "Height")
      height.type = "number"
      height.min = "64"
      height.max = "4096"
      height.dataset.recipeField = "height"
      model.append(visualField("Preferred checkpoint", checkpoint), visualField("Width", width), visualField("Height", height))
      const stacks = visualCard("Swarm stacks", "One preset or LoRA per line", true)
      const presets = visualTextarea(recipe.swarmPresets.join("\n"), "Cinematic\nNoir Polish")
      presets.dataset.recipeField = "swarmPresets"
      const loras = visualTextarea(formatVisualLoraLines(recipe.loraStack), "styles/noir.safetensors | 0.7 | trigger")
      loras.dataset.recipeField = "loraStack"
      stacks.append(visualField("Swarm preset stack", presets), visualField("LoRA stack · path | weight | trigger", loras))
      const prompts = visualCard("Prompt additions", "Layered before character and scene", true)
      const positive = visualTextarea(recipe.positiveAddition, "cinematic noir, hard rim light…")
      positive.dataset.recipeField = "positiveAddition"
      const negative = visualTextarea(recipe.negativeAddition, "flat lighting, pastel palette…")
      negative.dataset.recipeField = "negativeAddition"
      prompts.append(visualField("Positive addition", positive), visualField("Negative addition", negative))
      const scene = visualCard("Scene extraction style", "Future scene-tool instruction", true)
      const sceneInput = visualTextarea(recipe.sceneExtractionStyle, "Describe how chat scenes should be translated into image prompts…")
      sceneInput.dataset.recipeField = "sceneExtractionStyle"
      scene.appendChild(visualField("Instruction", sceneInput))
      grid.append(model, stacks, prompts, scene)
      shell.appendChild(grid)
      tab.root.replaceChildren(shell)
      const write = () => {
        currentPresetRecipe = cleanLoomVisualRecipe(recipe)
        editor.updateMetadata((current: unknown) => ({
          ...visualRecord(current),
          visualRecipe: currentPresetRecipe,
        }))
      }
      shell.addEventListener("input", (event) => {
        const target = event.target as HTMLInputElement | HTMLTextAreaElement
        const field = target.dataset.recipeField
        if (field === "preferredCheckpoint") recipe.preferredCheckpoint = target.value
        if (field === "swarmPresets") recipe.swarmPresets = target.value.split(/\r?\n/).map((item) => item.trim()).filter(Boolean)
        if (field === "loraStack") recipe.loraStack = parseVisualLoraLines(target.value)
        if (field === "positiveAddition") recipe.positiveAddition = target.value
        if (field === "negativeAddition") recipe.negativeAddition = target.value
        if (field === "width") recipe.width = target.value ? clamp(Number(target.value), 64, 4096) : null
        if (field === "height") recipe.height = target.value ? clamp(Number(target.value), 64, 4096) : null
        if (field === "sceneExtractionStyle") recipe.sceneExtractionStyle = target.value
        write()
      })
    }
    cleanups.push(ctx.ui.presetEditor.extension.onChange(() => render()))
    cleanups.push(tab.onActivate(() => render(true)))
    render()
    } catch {
      // Preset placement is optional and independently permission-gated.
    }
  }

  return {
    getPresetContext: () => ({ id: currentPresetId, name: currentPresetName, recipe: currentPresetRecipe }),
    refresh() {
      requestEditorOptions()
      rerenderCharacterEditor?.()
    },
    onBackendMessage(payload: any) {
          ...
          if (state?.open && String(state.characterId || "") === currentCharacterId) {
            ...
            if (!recipe.connectionId && editorOptions.connection?.id) {
              ctx.ui.characterEditor.updateExtensions((extensions: Record<string, any>) => ({
                ...
                [EXTENSION_NAMESPACE]: {
                  ...visualRecord(extensions?.[EXTENSION_NAMESPACE]),
                  visualBible: { ...recipe, connectionId: editorOptions.connection?.id || "" },
                },
              }))
            }
          }
...
class StudioController {
  ...
  private hadDraftAtOpen: boolean
  private readonly loomVisualContext: { id: string; name: string; recipe: LoomVisualRecipe | null }
  private readonly visualLaunchContext: VisualLaunchContext | null
  private visualContextApplied = false
  ...
  constructor(
    ...
    loomVisualContext: { id: string; name: string; recipe: LoomVisualRecipe | null } = { id: "", name: "", recipe: null },
    visualLaunchContext: VisualLaunchContext | null = null,
  ) {
    this.ctx = ctx
    this.modal = modal
    ...
    this.behavior = { ...behavior }
    this.onBehaviorChange = onBehaviorChange
    this.loomVisualContext = {
      id: loomVisualContext.id,
    ...
    this.hadDraftAtOpen = Boolean(this.pendingDraftRestore)
...
are removed when CSS is applied.</pre>
        ...
        <div class="ss-inspector" data-role="inspector" hidden>
          ...
          <aside class="ss-inspector-details">
            ...
            <section class="ss-use-lumiverse">
              <h4>Use in Lumiverse</h4>
              <div class="ss-use-lumiverse-actions">
                <button class="ss-button" data-action="set-character-avatar" disabled>Set as character avatar</button>
                <button class="ss-button" data-action="append-to-chat" disabled>Post to current chat</button>
                <button class="ss-button" data-action="set-chat-wallpaper" disabled>Use as chat wallpaper</button>
                <button class="ss-button" data-action="apply-output-palette" disabled>Use palette for Lumiverse</button>
              </div>
              <p class="ss-use-lumiverse-note" data-role="lumiverse-action-note">Actions become available for Lumiverse-owned outputs while a character chat is open.</p>
            </section>
  ...
  private bind(): void {
    ...
    this.root.addEventListener("click", (event) => {
      ...
      if (action === "set-character-avatar") void this.setCurrentAsCharacterAvatar()
      if (action === "apply-output-palette") this.applyCurrentOutputPalette()
      if (action === "set-chat-wallpaper") this.setCurrentAsChatWallpaper()
    ...
    switch (payload?.type) {
      case "bootstrap_result":
        ...
        this.state.activeCharacter = data.activeCharacter || null
        {
          ...
          this.state.characterVisual = launchMatches
            ? cleanCharacterVisualBible(launched?.visualBible)
            : Object.keys(visualRecord(data.characterVisual)).length
              ? cleanCharacterVisualBible(data.characterVisual)
              : null
          const draftCharacterId = String(this.pendingDraftRestore?.characterId || "")
          if (
            this.pendingDraftRestore
            && activeCharacterId
            && this.state.characterVisual
            && draftCharacterId !== activeCharacterId
          ) {
      ...
      case "output_appended_to_chat":
        ...
        break
      case "character_avatar_updated":
        this.setRunStatus(`Set this output as ${data.characterName || "the active character"}’s avatar.`)
        break
      case "output_palette_applied":
        this.setRunStatus("Applied this output’s palette to Lumiverse.")
        break
      case "chat_wallpaper_updated":
        this.setRunStatus("Set this output as the current chat wallpaper.")
        break
      case "output_deleted":
        this.acceptOutputPage(data)
        this.state.outputFolders = Array.isArray(data.folders) ? data.folders : this.state.outputFolders
  ...
  private renderPermissions(): void {
    ...
    if (!this.state.permissions.characters) missing.push("Characters (Visual Bible and avatar replacement)")
    if (!this.state.permissions.presets) missing.push("Presets (Loom visual recipes)")
    if (!this.state.permissions.theme) missing.push("App Manipulation (apply output palettes)")
  ...
  private acceptConnectionData(data: any): void {
    ...
    if (!this.hadDraftAtOpen && !this.visualContextApplied) this.applyVisualContext()
    this.updateFamilyChip()
    this.showMetadataError(data.metadataError || "")
    this.renderLoras()
    this.renderStack()
  ...
  private applyVisualContext(): void {
    this.visualContextApplied = true
    ...
    const joinUnique = (parts: Array<string | undefined>): string => {
      const seen = new Set<string>()
      const output: string[] = []
    ...
    this.get<HTMLTextAreaElement>('[data-role="positive"]').value = joinUnique([
    ...
    this.get<HTMLTextAreaElement>('[data-role="negative"]').value = joinUnique([
    ...
    if (requestedCheckpoint) {
      const normalize = (value: string) => value.toLowerCase().replace(/\\/g, "/").replace(/\.(safetensors|ckpt|pt)$/i, "").trim()
      const requested = normalize(requestedCheckpoint)
      const model = this.get<HTMLSelectElement>('[data-role="model"]')
      const match = [...model.options].find((option) => {
        const value = normalize(option.value)
        const label = normalize(option.textContent || "")
        return value === requested || label === requested || value.endsWith(`/${requested}`)
      })
    ...
    if (customCharacter?.defaultAspectRatio) {
      ...
      if ([...aspect.options].some((option) => option.value === customCharacter.defaultAspectRatio)) {
    ...
    const wantedPresets: SelectedPreset[] = [
      ...(loom?.swarmPresets || []).map((title) => ({ title, enabled: true })),
    ...
    this.state.selectedPresets = wantedPresets.flatMap((wanted) => {
      ...
      return [{ title: available?.title || wanted.title, enabled: Boolean(available && wanted.enabled) }]
    })

    const combinedLoras = cleanVisualLoras([...(loom?.loraStack || []), ...(customCharacter?.baseLoraStack || [])])
  ...
  private updateResolvedPresetSummary(): void {
    ...
    const characterLabel = this.state.characterVisual
      ? `Visuals: ${this.state.activeCharacter?.name || this.visualLaunchContext?.characterName || "Character"}`
      : ""
    const loomLabel = this.loomVisualContext.recipe
      ? this.loomVisualContext.name || "Loom visual style"
      : ""
    if (!enabled.length) {
  ...
  private async setCurrentAsCharacterAvatar(): Promise<void> {
    const image = this.state.currentImage
    if (!image?.id || !image.src || !this.state.activeChat?.character_id) return
    this.setRunStatus("Preparing the selected output as the active character avatar…")
    try {
      const response = await fetch(image.url || image.src, { credentials: "same-origin" })
      if (!response.ok) throw new Error(`Image request failed (${response.status}).`)
      const blob = await response.blob()
      if (!blob.type.startsWith("image/")) throw new Error("The selected output is not an image.")
      const dataUrl = await new Promise<string>((resolve, reject) => {
        const reader = new FileReader()
        reader.onload = () => resolve(String(reader.result || ""))
        reader.onerror = () => reject(reader.error || new Error("Could not read the selected output."))
        reader.readAsDataURL(blob)
      })
      this.send("set_output_as_character_avatar", {
        imageId: image.id,
        dataUrl,
        filename: image.label,
      })
    } catch (error) {
      this.setRunStatus(`Could not set the character avatar: ${error instanceof Error ? error.message : String(error)}`, true)
    }
  }

  private applyCurrentOutputPalette(): void {
    const image = this.state.currentImage
    if (!image?.id) return
    this.setRunStatus("Extracting a Lumiverse palette from the selected output…")
    this.send("apply_output_palette", { imageId: image.id })
  }

  private setCurrentAsChatWallpaper(): void {
    const image = this.state.currentImage
    if (!image?.id || !this.state.activeChat?.id || !this.state.permissions.wallpaper) return
    this.setRunStatus("Setting the selected output as this chat’s wallpaper…")
    this.send("set_output_as_chat_wallpaper", { imageId: image.id })
  }
  ...
  private renderOutputLibrary(): void {
    ...
    const appendFolder = (id: string, name: string, count: number, character = false) => {
    ...
    this.get<HTMLElement>('[data-role="library-title"]').textContent =
      selectedFolder
        ? selectedFolder.kind === "character" ? `Visuals · ${selectedFolder.name}` : selectedFolder.name
        : this.libraryFolderId === "__unfiled__" ? "Unfiled" : "All outputs"
  ...
  private updateAppendControls(): void {
    ...
    this.updateLumiverseOutputControls()
  }

  private updateLumiverseOutputControls(): void {
    const saved = Boolean(this.state.currentImage?.id)
    const hasChat = Boolean(this.state.activeChat?.id)
    const hasCharacter = Boolean(this.state.activeChat?.character_id)
    const avatar = this.root.querySelector<HTMLButtonElement>('[data-action="set-character-avatar"]')
    if (avatar) {
      avatar.disabled = !(saved && hasCharacter && this.state.permissions.characters)
      avatar.title = avatar.disabled
        ? "Requires a saved output, an active character chat, and Characters permission"
        : "Replace the active character card avatar with this output"
    }
    const palette = this.root.querySelector<HTMLButtonElement>('[data-action="apply-output-palette"]')
    if (palette) {
      palette.disabled = !(saved && this.state.permissions.theme && this.state.permissions.images)
      palette.title = palette.disabled
        ? "Requires a saved output plus Images and App Manipulation permissions"
        : "Apply an extension-scoped Lumiverse palette derived from this output"
    }
    const wallpaper = this.root.querySelector<HTMLButtonElement>('[data-action="set-chat-wallpaper"]')
    if (wallpaper) {
      wallpaper.disabled = !(saved && hasChat && this.state.permissions.wallpaper)
      wallpaper.title = this.state.permissions.wallpaper
        ? "Set this output as the current chat wallpaper"
        : "Waiting for Lumiverse’s public chat wallpaper helper"
    }
    const note = this.root.querySelector<HTMLElement>('[data-role="lumiverse-action-note"]')
    if (note) {
      note.textContent = this.state.permissions.wallpaper
        ? "Lumiverse actions preserve the saved image and its Swarm Studio generation metadata."
        : "Avatar, chat, and palette actions use public Spindle APIs. Wallpaper stays disabled until Lumiverse exposes its public chat-wallpaper helper."
    }
  }
...
export function setup(ctx: FrontendContext): () => void {
  ...
  let removeCustomStyle: (() => void) | null = appearance.customCss
    ...
    : null
  let visualEditors: ReturnType<typeof registerVisualRecipeEditors> | null = null
  ...
  const openStudio = (
    initialView: "studio" | "library" = "studio",
    visualLaunchContext: VisualLaunchContext | null = null,
    libraryFolderId = "",
  ) => {
    if (activeModal) {
    ...
    activeStudio = new StudioController(
      ...
      updateBehavior,
      miniplayer,
      visualEditors?.getPresetContext() || { id: "", name: "", recipe: null },
      visualLaunchContext,
    )
    activeStudio.setAppearance(appearance)
  ...
  try {
    ...
    // Editor placements disappear automatically when their permissions are not
    // granted. Studio remains usable with its existing extension-local tools.
  }
  ...
  const unsubscribeMessages = ctx.onBackendMessage((payload: any) => {
    visualEditors?.onBackendMessage(payload)
