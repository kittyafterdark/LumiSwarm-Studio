...
interface CharacterVisualBible {
  ...
  outfits: VisualProfile[]
  activeOutfitId: string
  styles: VisualProfile[]
...
function registerVisualRecipeEditors(ctx: FrontendContext): {
  ...
  if (typeof ctx.ui?.registerCharacterEditorTab === "function" && ctx.ui.characterEditor) {
    ...
    const render = (force = false) => {
      ...
      const buildProfiles = (kind: "outfits" | "styles", title: string, items: VisualProfile[], activeId: string) => {
        ...
        for (const [index, profile] of items.entries()) {
          ...
          for (const [field, value, placeholder] of [
            ["name", profile.name, kind === "outfits" ? "Outfit name" : "Style name"],
      ...
      grid.append(
        ...
        references,
        loras,
        buildProfiles("outfits", "Outfit profiles", recipe.outfits, recipe.activeOutfitId),
        buildProfiles("styles", "Style profiles", recipe.styles, recipe.activeStyleId),
        gallery,
      )
      ...
      shell.addEventListener("input", (event) => {
        ...
        const kind = target.dataset.visualProfileKind as "outfits" | "styles" | undefined
        ...
        if (kind && Number.isInteger(index) && profileField && recipe[kind][index]) {
          ;(recipe[kind][index] as any)[profileField] = target.value
        }
        write()
      })
      ...
      shell.addEventListener("click", (event) => {
        ...
        if (!button) return
        const addKind = button.dataset.visualAddProfile as "outfits" | "styles" | undefined
        const removeKind = button.dataset.visualRemoveProfile as "outfits" | "styles" | undefined
        if (addKind) {
          recipe[addKind].push({ id: crypto.randomUUID(), name: addKind === "outfits" ? "New outfit" : "New style", prompt: "", negativePrompt: "" })
          write(true)
        }
        if (removeKind) {
...
class StudioController {
  ...
  private readonly hadDraftAtOpen: boolean
...
are removed when CSS is applied.</pre>
  ...
  onMessage(payload: any): void {
    ...
    switch (payload?.type) {
      case "bootstrap_result":
        ...
        this.state.characterVisual = Object.keys(visualRecord(data.characterVisual)).length
  ...
  private applyVisualContext(): void {
    ...
    if (character?.defaultAspectRatio) {
      ...
      if ([...aspect.options].some((option) => option.value === character.defaultAspectRatio)) {
    ...
    this.state.selectedPresets = wantedPresets.map((wanted) => {
      ...
      return { title: available?.title || wanted, enabled: Boolean(available) }
    })

    const combinedLoras = cleanVisualLoras([...(loom?.loraStack || []), ...(character?.baseLoraStack || [])])
  ...
  private updateResolvedPresetSummary(): void {
    ...
    if (!enabled.length) {
      ...
      const recipeLabel = this.loomVisualContext.recipe
        ? this.loomVisualContext.name || "Loom visual style"
        : this.state.characterVisual
          ? this.state.activeCharacter?.name || "Character bible"
          : ""
