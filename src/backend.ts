  ...
  const identityGuidance = [
    characterTags
      ? automation.autoPrintCharacterPositive
        ? `CHARACTER 1 IDENTITY — automatically prepended for character="active"; do not repeat it:\n${characterTags}`
        : `CHARACTER 1 IDENTITY — available for character="active", but not automatically prepended. Select only the concrete appearance tags needed for the visible subject(s) and include those tags in the request body; this block may contain multiple NPC definitions:\n${characterTags}`
  ...
  if (characterBase && personaBase) {
    const identityLines = [
      automation.autoPrintCharacterPositive && !contains(characterBase)
        ? `character 1 identity: ${characterBase}`
        : "",
